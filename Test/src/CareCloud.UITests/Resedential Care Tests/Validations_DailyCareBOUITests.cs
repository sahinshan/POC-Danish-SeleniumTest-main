...
namespace CareCloud.UITests.Resedential_Care_Tests
{
    ...
    public class Validations_DailyCareBOUITests : TestBase
    {
        #region Properties
        ...
        private string _loginUsername = "Validationy_User";
        ...
        public void TestInitializationMethod()
        {
            ...
            try
            {
                ...
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CareCloud Mobile User (Rostered User)")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Alert/Hazard Module (Team View)")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Can View Contact")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Can View People We Support")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Can View Prospect ")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Can View Referral")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Care Planning (Team Edit)")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Care Provider Care Plan Forms (Team Edit)")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Care Provider Reference Data (Team View)")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Care Provider Scheduling Setup View Only")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Care Provider Settings View")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW CareDirector User")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Core Reference Data (Team Edit)")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Daily Care (Team Edit)")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Person (Team Edit)")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Person Module (Team Edit)")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW Timeline Record (Team View)")[0]);
                userSecProfiles.Add(dbHelper.securityProfile.GetSecurityProfileByName("CW User Work Schedules (Team View)")[0]);
                ...
                _DefaultTeam = dbHelper.team.GetTeamIdByName("Residential Mobile Tenant (Care Cloud)").FirstOrDefault();
                ...
                _bookingType5 = commonMethodsDB.CreateCPBookingType("ValidationBookingType-005", 5, 960, new TimeSpan(6, 0, 0), new TimeSpan(22, 0, 0), 2, false, 1440);

                #endregion

                #region Provider 1

                _providereId1 = commonMethodsDB.CreateProvider("ValidationProvider-001", _careDirectorQA_TeamId, 13, true); //create a "Residential Establishment" provider
        ...
        [Test]
        [Property("JiraIssueID", "ACC-8186")]
        [Description("Care Cloud Mobile app -Resedential Care -As a Rostered User I want the time I spent with a resident to be considered when entering a care noteSo that I can’t enter something that hasn’t happened yet .Step1-Step8")]
        public void Validation_Mobility_UITest001()
        {
            ...
            var todayDate = DateTime.Today.AddHours(DateTime.Now.Hour);
            var minutes= DateTime.Today.AddHours(DateTime.Now.Minute-2);
            ...
            resedentDetailsPage
                ...
                .TapCareTab()
                .TapAddCareButton()
                .TapBO(_personFullname, _personnumber, "Mobility")
                .TapCareConsentgivenYes(_personFullname, _personnumber);

            mobilityRecordPage
                .WaitForMobilityRecordPageLookupToLoad(_personFullname, _personnumber)
                .InsertTimeCareGiven();
            ...
            timePickerPopUp
                ...
                .setInputMinuteTime(minutes.ToString("mm"))
                .TapTimeSetButton();
