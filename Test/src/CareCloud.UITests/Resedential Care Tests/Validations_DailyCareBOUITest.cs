...
namespace CareCloud.UITests.Resedential_Care_Tests
{
    ...
    public class Validations_DailyCareBOUITest : TestBase
    {
        #region Properties
        ...
        public void Validation_Mobility_UITest001()
        {
            ...
            var minutes = DateTime.Today.AddMinutes(DateTime.Now.Minute + 3);
            ...
            mobilityRecordPage
               ...
               .InsertTimeSpent()
               .validateTimeCareGivenValidation1("You cannot select future time.")
               .validateTimeCareGivenValidation2("Time care was given and time spent with person cannot be greater than the current time when combined.")
               .validateTimeSpentValidation("Time care was given and time spent with person cannot be greater than the current time when combined.");


        }

        [Test]
        [Property("JiraIssueID", "ACC-8187")]
        [Description("Care Cloud Mobile app -Resedential Care -As a Rostered User I want the time I spent with a resident to be considered when entering a care noteSo that I can’t enter something that hasn’t happened yet .Step1-Step8")]
        public void Validation_Repositioning_UITest002()
        {
            _providerName = (string)dbHelper.provider.GetProviderByID(_providereId1, "name")["name"];
            _personFullname = (string)dbHelper.person.GetPersonById(_personID, "fullname")["fullname"];
            _personnumber = (int)dbHelper.person.GetPersonById(_personID, "personnumber")["personnumber"];
        ...
        [Test]
        [Property("JiraIssueID", "ACC-8188")]
        [Description("Care Cloud Mobile app -Resedential Care -As a Rostered User I want the time I spent with a resident to be considered when entering a care noteSo that I can’t enter something that hasn’t happened yet .Step1-Step8")]
        public void Validation_WelfareCheck_UITest003()
        {
            _providerName = (string)dbHelper.provider.GetProviderByID(_providereId1, "name")["name"];
            _personFullname = (string)dbHelper.person.GetPersonById(_personID, "fullname")["fullname"];
            _personnumber = (int)dbHelper.person.GetPersonById(_personID, "personnumber")["personnumber"];
