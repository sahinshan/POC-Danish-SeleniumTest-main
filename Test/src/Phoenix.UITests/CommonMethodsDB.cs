...
namespace Phoenix.UITests
{
    public class CommonMethodsDB
    {
        ...
        internal Framework.FileSystem.FileIOHelper fileIOHelper;
        ...
        public CommonMethodsDB(DBHelper.DatabaseHelper _dbHelper, Framework.FileSystem.FileIOHelper _fileIOHelper, TestContext _testContext)
        {
            dbHelper = _dbHelper;
        ...
        public Guid CreateSystemUserRecord(string username, string firstName, string lastName, string password, Guid businessUnitId, Guid teamId, Guid _languageId, Guid _authenticationproviderid, List<Guid> SecurityProfiles, int employeetypeid)
        {
            #region System User
            ...
            if (!dbHelper.systemUser.GetSystemUserByUserName(username).Any())
                _userId = dbHelper.systemUser.CreateSystemUser(username, firstName, lastName, firstName + " " + lastName, password, username + "@somemail.com", username + "@otheremail.com", "GMT Standard Time", null, null, _languageId, _authenticationproviderid, businessUnitId, teamId, DateTime.Now.Date, false, true, null, employeetypeid);
        ...
        public Guid CreateProvider(string name, Guid ownerid, int providertypeid, bool enablescheduling, DateTime addressstartdate, int addresstypeid, string propertyname, string propertyNo, string street, string district, string town, string county, string postcode)
        {
            if (!dbHelper.provider.GetProviderByName(name).Any())
                dbHelper.provider.CreateProvider(name, ownerid, providertypeid, enablescheduling, addressstartdate, addresstypeid, propertyname, propertyNo, street, district, town, county, postcode);
            return dbHelper.provider.GetProviderByName(name)[0];
        }
        ...
        public Guid CreateCarePhysicalLocation(string Name, DateTime StartDate, Guid OwnerId, List<int> value)
        {
            ...
            if (!recordExists)
                return dbHelper.carePhysicalLocation.CreateCarePhysicalLocation(Name, StartDate, OwnerId, value);

            return dbHelper.carePhysicalLocation.GetByName(Name)[0];
        }

        public Guid CreateCarePhysicalLocation(string Name, DateTime StartDate, Guid OwnerId, Dictionary<int, string> value)
        {
            var recordExists = dbHelper.carePhysicalLocation.GetByName(Name).Any();
        ...
        public Guid CreateCareEquipment(string Name, DateTime StartDate, Guid OwnerId, List<int> value)
        {
            var recordExists = dbHelper.careEquipment.GetByName(Name).Any();

            if (!recordExists)
                return dbHelper.careEquipment.CreateCareEquipment(Name, StartDate, OwnerId, value);

            return dbHelper.careEquipment.GetByName(Name)[0];
        }

        public Guid CreateMobilityDistanceUnit(string Name, DateTime StartDate, Guid OwnerId)
        {
            var recordExists = dbHelper.mobilityDistanceUnit.GetByName(Name).Any();

            if (!recordExists)
                return dbHelper.mobilityDistanceUnit.CreateMobilityDistanceUnit(Name, StartDate, OwnerId);

            return dbHelper.mobilityDistanceUnit.GetByName(Name)[0];
        }

        public Guid CreateCareAssistanceNeeded(string Name, DateTime StartDate, Guid OwnerId)
        {
            var recordExists = dbHelper.careAssistanceNeeded.GetByName(Name).Any();

            if (!recordExists)
                return dbHelper.careAssistanceNeeded.CreateCareAssistanceNeeded(Name, StartDate, OwnerId);

            return dbHelper.careAssistanceNeeded.GetByName(Name)[0];
        }

        public Guid CreateCareWellbeing(string Name, DateTime StartDate, Guid OwnerId)
        {
            var recordExists = dbHelper.careWellbeing.GetByName(Name).Any();

            if (!recordExists)
                return dbHelper.careWellbeing.CreateCareWellbeing(Name, StartDate, OwnerId);

            return dbHelper.careWellbeing.GetByName(Name)[0];
        }
        ...
        public Guid CreateCommunityAndClinicTeam(Guid Ownerid, Guid Providerid, Guid TeamId, string Title, string Comments)
        {
            ...
            if (!dbHelper.communityAndClinicTeam.GetByTitle(Title).Any())
                return dbHelper.communityAndClinicTeam.CreateCommunityAndClinicTeam(Ownerid, Providerid, TeamId, Title, Comments);
        ...
        public Guid CreateMaritalStatus(string Name, DateTime startdate, Guid Ownerid)
        {
            if (!dbHelper.maritalStatus.GetMaritalStatusIdByName(Name).Any())
                return dbHelper.maritalStatus.CreateMaritalStatus(Name, startdate, Ownerid);
        ...
        public Guid CreateCarePlanTypeId(string Name, DateTime startdate, Guid Ownerid)
        {
            if (!dbHelper.carePlanType.GetByName(Name).Any())
                return dbHelper.carePlanType.CreateCarePlanTypeId(Name, startdate, Ownerid);
        ...
        public Guid CreateContactPresentingPriority(Guid Ownerid, string Name)
        {
            if (!dbHelper.contactPresentingPriority.GetByName(Name).Any())
                return dbHelper.contactPresentingPriority.CreateContactPresentingPriority(Ownerid, Name, null, null, new DateTime(2021, 10, 10));
        ...
        public Guid CreateCasePriority(string Name, DateTime StartDate, int CasePriorityCategoryId, Guid OwnerId)
        {
            if (!dbHelper.casePriority.GetByName(Name).Any())
                return dbHelper.casePriority.CreateCasePriority(Name, StartDate, CasePriorityCategoryId, OwnerId);
        ...
        public Guid CreateFosteringExperience(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.fosteringExperience.GetByName(Name).Any())
                return dbHelper.fosteringExperience.CreateFosteringExperience(Name, StartDate, OwnerId);
        ...
        public Guid CreateChildInNeedCode(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.childInNeedCode.GetByName(Name).Any())
                return dbHelper.childInNeedCode.CreateChildInNeedCode(Name, StartDate, OwnerId);
        ...
        public Guid CreateContactType(Guid OwnerId, string Name, DateTime StartDate, bool ValidForInitialContact)
        {
            if (!dbHelper.contactType.GetByName(Name).Any())
                return dbHelper.contactType.CreateContactType(OwnerId, Name, StartDate, ValidForInitialContact);
        ...
        public Guid CreateContactStatus(Guid OwnerId, string Name, string Code, DateTime StartDate, int CategoryId, bool ValidForReopening)
        {
            if (!dbHelper.contactStatus.GetByName(Name).Any())
                return dbHelper.contactStatus.CreateContactStatus(OwnerId, Name, Code, StartDate, CategoryId, ValidForReopening);
        ...
        public Guid CreateErrorManagementReason(string Name, DateTime StartDate, int ApplicableToId, Guid OwnerId, bool Inactive = false)
        {
            if (!dbHelper.errorManagementReason.GetByName(Name).Any())
                return dbHelper.errorManagementReason.CreateErrorManagementReason(Name, StartDate, ApplicableToId, OwnerId, Inactive);
        ...
        public Guid CreatePaymentTypeCode(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.paymentTypeCode.GetByName(Name).Any())
                return dbHelper.paymentTypeCode.CreatePaymentTypeCode(Name, StartDate, OwnerId);
        ...
        public Guid CreateVATCode(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.vatCode.GetByName(Name).Any())
                return dbHelper.vatCode.CreateVATCode(Name, StartDate, OwnerId);
        ...
        public Guid CreateProviderBatchGrouping(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.providerBatchGrouping.GetByName(Name).Any())
                return dbHelper.providerBatchGrouping.CreateProviderBatchGrouping(Name, StartDate, OwnerId);
        ...
        public Guid CreateFormDelayReason(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.formDelayReason.GetByName(Name).Any())
                return dbHelper.formDelayReason.CreateFormDelayReason(Name, StartDate, OwnerId);
        ...
        public Guid CreateInpatientAdmissionMethod(string name, Guid ownerid, Guid OwningBusinessUnitId, DateTime startDate)
        {
            ...
            if (!inpatientAdmissionMethodExists)
                return dbHelper.inpatientAdmissionMethod.CreateAdmissionMethod(name, ownerid, OwningBusinessUnitId, startDate);
        ...
        public Guid CreateCaseReopenReason(Guid ownerid, string Name, string Code, DateTime StartDate, int BusinessTypeId)
        {
            if (!dbHelper.caseReopenReason.GetByName(Name).Any())
                return dbHelper.caseReopenReason.CreateCaseReopenReason(ownerid, Name, Code, StartDate, BusinessTypeId);
        ...
        public Guid CreateAttachDocumentType(Guid OwnerId, string Name, DateTime StartDate)
        {
            if (!dbHelper.attachDocumentType.GetAttachDocumentTypeIdByName(Name).Any())
                return dbHelper.attachDocumentType.CreateAttachDocumentType(OwnerId, Name, StartDate);
        ...
        public Guid CreateAttachDocumentSubType(Guid OwnerId, string Name, DateTime StartDate, Guid AttachDocumentTypeId)
        {
            if (!dbHelper.attachDocumentSubType.GetAttachDocumentSubTypeIdByName(Name).Any())
                return dbHelper.attachDocumentSubType.CreateAttachDocumentSubType(OwnerId, Name, StartDate, AttachDocumentTypeId);
        ...
        public Guid CreateRTTPathwaySetup(Guid OwnerId, string Name, DateTime StartDate, int firstappointmentnolaterthan, int warnafter, int breachoccursafter)
        {
            if (!dbHelper.rttPathwaySetup.GetByName(Name).Any())
                return dbHelper.rttPathwaySetup.CreateRTTPathwaySetup(OwnerId, Name, StartDate, firstappointmentnolaterthan, warnafter, breachoccursafter);
        ...
        public Guid CreateJobRoleType(Guid OwnerId, string Name, DateTime StartDate, int isconsultantid)
        {
            if (!dbHelper.jobRoleType.GetByName(Name).Any())
                return dbHelper.jobRoleType.CreateJobRoleType(OwnerId, Name, StartDate, isconsultantid);
        ...
        public Guid CreateRTTTreatmentStatus(Guid OwnerId, Guid OwningBusinessUnitId, string Name, string GovCode, int RTTWaitingStatusId, DateTime StartDate, string Description)
        {
            if (!dbHelper.rttTreatmentStatus.GetByName(Name).Any())
                return dbHelper.rttTreatmentStatus.CreateRTTTreatmentStatus(OwnerId, OwningBusinessUnitId, Name, GovCode, RTTWaitingStatusId, StartDate, Description);
        ...
        public Guid CreateProfessional(Guid OwnerId, Guid ProfessionTypeId, string Title, string FirstName, string LastName, string Email = "")
        {
            if (!dbHelper.professional.GetProfessionalIdByFirstName(FirstName).Any())
                return dbHelper.professional.CreateProfessional(OwnerId, ProfessionTypeId, Title, FirstName, LastName, Email);
        ...
        public Guid CreateClinicalRiskFactorType(Guid OwnerId, string Name, DateTime StartDate, DateTime? EndDate)
        {
            if (!dbHelper.clinicalRiskFactorType.GetByName(Name).Any())
                return dbHelper.clinicalRiskFactorType.CreateClinicalRiskFactorType(OwnerId, Name, StartDate, EndDate);
        ...
        public Guid CreateClinicalRiskFactorSubType(Guid OwnerId, string Name, DateTime StartDate, Guid ClinicalRiskFactorTypeId)
        {
            if (!dbHelper.clinicalRiskFactorSubType.GetByName(Name).Any())
                return dbHelper.clinicalRiskFactorSubType.CreateClinicalRiskFactorSubType(OwnerId, Name, StartDate, ClinicalRiskFactorTypeId);
        ...
        public Guid CreateClinicalRiskfactorEndReason(Guid OwnerId, string Name, DateTime StartDate)
        {
            if (!dbHelper.clinicalRiskfactorEndReason.GetByName(Name).Any())
                return dbHelper.clinicalRiskfactorEndReason.CreateClinicalRiskfactorEndReason(OwnerId, Name, StartDate);
        ...
        public Guid CreateClinicalRiskLevel(Guid OwnerId, string Name, DateTime StartDate)
        {
            if (!dbHelper.clinicalRiskLevel.GetByName(Name).Any())
                return dbHelper.clinicalRiskLevel.CreateClinicalRiskLevel(OwnerId, Name, StartDate);
            ...
            bool allowplaceofextensions = false, bool allowrenewalbeforesectionend = false)
        {
            if (!dbHelper.mhaSectionSetup.GetMHASectionSetupByCode(code).Any())
                return dbHelper.mhaSectionSetup.CreateMHASectionSetup(sectionName, code, ownerId, description, informal,
        ...
        public Guid CreateGestationPeriodEndReason(Guid OwnerId, string Name, DateTime StartDate, bool Inactive = false)
        {
            if (!dbHelper.gestationPeriodEndReason.GetByName(Name).Any())
                return dbHelper.gestationPeriodEndReason.CreateGestationPeriodEndReason(OwnerId, Name, StartDate, Inactive);
        ...
        public Guid CreateClinicalRiskStatus(Guid OwnerId, string Name, DateTime StartDate, DateTime? EndDate = null)
        {
            if (!dbHelper.clinicalRiskStatus.GetByName(Name).Any())
                return dbHelper.clinicalRiskStatus.CreateClinicalRiskStatus(OwnerId, Name, StartDate, EndDate);
        ...
        public Guid CreateHealthIssueTypeRecord(Guid OwnerId, String Name, int Inactive, int ValidForExport, DateTime StartDate, int? HealthIssueCategoryId = null)
        {
            if (!dbHelper.healthIssueType.GetByHealthIssueTypeName(Name).Any())
                return dbHelper.healthIssueType.CreateHealthIssueTypeRecord(OwnerId, Name, Inactive, ValidForExport, StartDate, HealthIssueCategoryId);
        ...
        public Guid CreateDisabilityType(Guid OwnerId, string Name, DateTime StartDate, bool ApplicableForMHSDS = false, bool IsNoneKnown = false, bool ValidForExport = false, bool Inactive = false)
        {
            if (!dbHelper.disabilityType.GetByDisabilityTypeName(Name).Any())
                return dbHelper.disabilityType.CreateDisabilityType(OwnerId, Name, StartDate, ApplicableForMHSDS, IsNoneKnown, ValidForExport, Inactive);
        ...
        public Guid CreateImpairmentType(Guid OwnerId, string Name, DateTime StartDate)
        {
            if (!dbHelper.impairmentType.GetByName(Name).Any())
                return dbHelper.impairmentType.CreateImpairmentType(OwnerId, Name, StartDate);
        ...
        public Guid CreateImmunisationType(Guid OwnerId, string Name, DateTime StartDate, bool ValidForExport = false, bool Inactive = false)
        {
            if (!dbHelper.immunisationType.GetByName(Name).Any())
                return dbHelper.immunisationType.CreateImmunisationType(OwnerId, Name, StartDate, ValidForExport, Inactive);
        ...
        public Guid CreateProfessionType(Guid OwnerId, string Name, DateTime StartDate)
        {
            if (!dbHelper.professionType.GetByName(Name).Any())
                return dbHelper.professionType.CreateProfessionType(OwnerId, Name, StartDate);
        ...
        public Guid CreateExtractName(Guid OwnerId, String Name, DateTime StartDate, bool IsUsedInSupplierPayments, bool IsUsedInCarerPayments, bool IsUsedInDebtors)
        {
            if (!dbHelper.extractName.GetByName(Name).Any())
                return dbHelper.extractName.CreateExtractName(OwnerId, Name, StartDate, IsUsedInSupplierPayments, IsUsedInCarerPayments, IsUsedInDebtors);
        ...
        public Guid CreateInvoiceBy(Guid OwnerId, string Name, DateTime StartDate, bool IsUsedInSupplierPayments, bool IsUsedInCarerPayments, bool IsUsedInDebtors, bool ValidForExport = false, bool Inactive = false)
        {
            if (!dbHelper.invoiceBy.GetByName(Name).Any())
                return dbHelper.invoiceBy.CreateInvoiceBy(OwnerId, Name, StartDate, IsUsedInSupplierPayments, IsUsedInCarerPayments, IsUsedInDebtors, ValidForExport, Inactive);
        ...
        public Guid CreateStaffRecruitmentItem(Guid ownerid, string Name, DateTime StartDate, int? compliancerecurrenceid = null)
        {
            if (!dbHelper.staffRecruitmentItem.GetByName(Name).Any())
                return dbHelper.staffRecruitmentItem.CreateStaffRecruitmentItem(ownerid, Name, StartDate, compliancerecurrenceid);
        ...
        public Guid CreateServicePackageType(Guid OwnerId, string Name, DateTime StartDate, DateTime? EndDate = null, bool inactive = false, bool validforexport = false)
        {
            if (!dbHelper.servicePackageType.GetByName(Name).Any())
                return dbHelper.servicePackageType.CreateServicePackageType(OwnerId, Name, StartDate, EndDate, inactive, validforexport);
        ...
        public Guid CreateAlertAndHazardType(Guid OwnerId, string Name, DateTime StartDate, bool validforswallowassessment = false, bool Inactive = false)
        {
            if (!dbHelper.alertAndHazardType.GetByName(Name).Any())
                return dbHelper.alertAndHazardType.CreateAlertAndHazardType(OwnerId, Name, StartDate, validforswallowassessment, Inactive);
        ...
        public Guid CreateFinanceGeneralSettingsIfNeeded(Guid OwnerId, int FinanceModuleId, bool IsFullFinancialYear, DateTime FinanceTransactionsUpTo)
        {
            if (!dbHelper.financeGeneralSettings.GetByOwnerIdAndFinanceModuleId(OwnerId, FinanceModuleId).Any())
                return dbHelper.financeGeneralSettings.CreateFinanceGeneralSettings(OwnerId, FinanceModuleId, IsFullFinancialYear, FinanceTransactionsUpTo);
        ...
        public Guid CreateFinanceGeneralSettingsIfNeeded(Guid OwnerId, Guid BusinessUnitId, int FinanceModuleId, bool IsFullFinancialYear, DateTime FinanceTransactionsUpTo)
        {
            if (!dbHelper.financeGeneralSettings.GetByBusinessUnitIdAndFinanceModuleId(OwnerId, BusinessUnitId, FinanceModuleId).Any())
                return dbHelper.financeGeneralSettings.CreateFinanceGeneralSettings(OwnerId, BusinessUnitId, FinanceModuleId, IsFullFinancialYear, FinanceTransactionsUpTo);
        ...
        public Guid CreateLACReviewType(Guid ownerid, string name, DateTime startdate)
        {
            if (!dbHelper.lacReviewType.GetByName(name).Any())
                return dbHelper.lacReviewType.CreateLACReviewType(ownerid, name, startdate);
        ...
        public Guid CreateLanguage(string Name, Guid ownerid, string Code, string govcode, DateTime startdate, DateTime? enddate)
        {
            if (!dbHelper.language.GetLanguageIdByName(Name).Any())
                return dbHelper.language.CreateLanguage(Name, ownerid, Code, govcode, startdate, enddate);
        ...
        public Guid CreateLanguage(Guid PrimaryKeyId, string Name, Guid ownerid, string Code, string govcode, DateTime startdate, DateTime? enddate)
        {
            ...
            if (!fields.ContainsKey(dbHelper.language.PrimaryKeyName.ToLower()))
                return dbHelper.language.CreateLanguage(PrimaryKeyId, Name, ownerid, Code, govcode, startdate, enddate);
        ...
        public Guid CreateLanguageFluency(Guid PrimaryKeyId, string Name, Guid ownerid, DateTime startdate)
        {
            ...
            if (!fields.ContainsKey(dbHelper.languageFluency.PrimaryKeyName.ToLower()))
                return dbHelper.languageFluency.CreateLanguageFluency(PrimaryKeyId, Name, ownerid, startdate);
        ...
        public Guid CreateDuplicateDetectionRule(string Name, string Description, Guid RecordTypeId, bool Published, bool ExcludeInactiveMatchingRecords)
        {
            if (!dbHelper.duplicateDetectionRule.GetByName(Name).Any())
                return dbHelper.duplicateDetectionRule.CreateDuplicateDetectionRule(Name, Description, RecordTypeId, Published, ExcludeInactiveMatchingRecords);
        ...
        public Guid CreateDuplicateDetectionCondition(string name, Guid duplicatedetectionruleid, int CriterionId, int? nofcharacters, bool IgnoreBlankValues, Guid BusinessObjectFieldId)
        {
            if (!dbHelper.duplicateDetectionCondition.GetByName(name, duplicatedetectionruleid).Any())
                return dbHelper.duplicateDetectionCondition.CreateDuplicateDetectionCondition(name, duplicatedetectionruleid, CriterionId, nofcharacters, IgnoreBlankValues, BusinessObjectFieldId);
            ...
            string VatGlCode = "", string ExtractReference = "")
        {
            if (!dbHelper.financeExtractSetup.GetByBusinessUnitIdAndFinanceModuleId(OwnerId, BusinessUnitId, FinanceModuleId).Any())
                return dbHelper.financeExtractSetup.CreateFinanceExtractSetup(OwnerId, BusinessUnitId, FinanceModuleId, StartDate, StartTime, ExtractNameId, ExtractFrequencyId,
        ...
        {
            if (!dbHelper.employmentContractType.GetByName(name).Any())
                return dbHelper.employmentContractType.CreateEmploymentContractType(ownerid, name, code, govcode, StartDate);
            return dbHelper.employmentContractType.GetByName(name)[0];
        }
        ...
        {
            if (!dbHelper.careProviderContractScheme.GetCareProviderContractSchemeByName(name).Any())
                return dbHelper.careProviderContractScheme.CreateCareProviderContractScheme(ownerid, responsibleuserid, owningbusinessunitid, name, startdate, code, EstablishmentId, FunderId, sundriesapply, selffunding);
            return dbHelper.careProviderContractScheme.GetCareProviderContractSchemeByName(name)[0];
        }
            ...
            int diaryBookingsValidityId, int regularBookingsValidityId, bool countsTowardsPeriodWorked)
        {
            if (!dbHelper.availabilityTypes.GetAvailabilityTypeByName(name).Any())
                return dbHelper.availabilityTypes.CreateAvailabilityType(name, precedence, validForExport, ownerId, diaryBookingsValidityId, regularBookingsValidityId, countsTowardsPeriodWorked);
        ...
        public Guid CreateCareTask(Guid ownerid, Guid OwningBusinessUnitId, string name, Int32 code, DateTime startdate)
        {
            if (!dbHelper.careTask.GetByName(name).Any())
                return dbHelper.careTask.CreateCareTask(ownerid, OwningBusinessUnitId, name, code, startdate);
        ...
        public Guid CreateMashStatus(string name, int mashstatuscategoryid, DateTime startdate, Guid ownerid)
        {
            if (!dbHelper.mashStatus.GetByName(name).Any())
                return dbHelper.mashStatus.CreateMASHStatus(name, mashstatuscategoryid, startdate, ownerid);
        ...
        public Guid CreateCareProviderRateUnit(Guid ownerid, string Name, DateTime startdate, int code, bool timeanddays = false, bool bandedrates = false, bool oneoff = false, bool decimalplaces = false)
        {
            if (!dbHelper.careProviderRateUnit.GetByName(Name).Any())
                return dbHelper.careProviderRateUnit.CreateCareProviderRateUnit(ownerid, Name, startdate, code, timeanddays, bandedrates, oneoff, decimalplaces);
            else
            {
                var recordID = dbHelper.careProviderRateUnit.GetByName(Name)[0];
                dbHelper.careProviderRateUnit.UpdateTimeAndDays(recordID, timeanddays);
                dbHelper.careProviderRateUnit.UpdateOneOff(recordID, oneoff);
        ...
        public Guid CreateCareProviderBatchGrouping(Guid ownerid, string Name, DateTime startdate, int code)
        {
            if (!dbHelper.careProviderBatchGrouping.GetByName(Name).Any())
                return dbHelper.careProviderBatchGrouping.CreateCareProviderBatchGrouping(ownerid, Name, startdate, code);
        ...
        public Guid CreateCareProviderVatCode(Guid ownerid, string Name, DateTime startdate, int code)
        {
            if (!dbHelper.careProviderVatCode.GetByName(Name).Any())
                return dbHelper.careProviderVatCode.CreateCareProviderVatCode(ownerid, Name, startdate, code);
        ...
        public Guid CreateCPBookingType(string name, int BookingTypeClassId, int? Duration, TimeSpan? DefaultStartTime, TimeSpan? DefaultEndTime,
            int WorkingContractedTime, bool? IsAbsence = null, int? capduration = null, DateTime? validfromdate = null, DateTime? validtodate = null, int? cpbookingchargetypeid = null)
        {
        ...
        public Guid CreateCPPersonAbsenceReason(Guid ownerid, string Name, DateTime startdate, int code)
        {
            if (!dbHelper.cpPersonAbsenceReason.GetByName(Name).Any())
                return dbHelper.cpPersonAbsenceReason.CreateCPPersonAbsenceReason(ownerid, Name, startdate, code);
            return dbHelper.cpPersonAbsenceReason.GetByName(Name)[0];
        }


        public Guid CreateCareProviderCarePeriodSetup(Guid ownerid, string title, TimeSpan starttime)
        {
            if (!dbHelper.careProviderCarePeriodSetUp.GetByTitle(title).Any())
                return dbHelper.careProviderCarePeriodSetUp.CreateCareProviderCarePeriod(ownerid, title, starttime);

            return dbHelper.careProviderCarePeriodSetUp.GetByTitle(title)[0];
        }
        ...
        public Guid CreateCareProviderPersonContractServiceEndReason(Guid ownerid, string Name, DateTime startdate, int code)
        {
            if (!dbHelper.careProviderPersonContractServiceEndReason.GetByName(Name).Any())
                return dbHelper.careProviderPersonContractServiceEndReason.CreateCareProviderPersonContractServiceEndReason(ownerid, Name, startdate, code);
        ...
        public Guid CreateAddressBorough(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.addressBorough.GetAddressBoroughByName(Name).Any())
                return dbHelper.addressBorough.CreateAddressBorough(Name, StartDate, OwnerId);
        ...
        public Guid CreateAddressWard(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.addressWard.GetAddressWardByName(Name).Any())
                return dbHelper.addressWard.CreateAddressWard(Name, StartDate, OwnerId);
        ...
        public Guid CreatePersonTargetGroup(Guid ownerid, Guid OwningBusinessUnitId, string Name, int Code, DateTime StartDate, DateTime? EndDate, bool Inactive = false, bool ValidForExport = false)
        {
            if (!dbHelper.personTargetGroup.GetByName(Name).Any())
                return dbHelper.personTargetGroup.CreatePersonTargetGroup(ownerid, OwningBusinessUnitId, Name, Code, StartDate, EndDate, Inactive, ValidForExport);
        ...
        public Guid CreateAccommodationType(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.accommodationType.GetByName(Name).Any())
                return dbHelper.accommodationType.CreateAccommodationType(Name, StartDate, OwnerId);
        ...
        public Guid CreateLowerSuperOutputArea(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.lowerSuperOutputArea.GetByName(Name).Any())
                return dbHelper.lowerSuperOutputArea.CreateLowerSuperOutputArea(Name, StartDate, OwnerId);
        ...
        public Guid CreateModeOfCommunication(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.modeOfCommunication.GetByName(Name).Any())
                return dbHelper.modeOfCommunication.CreateModeOfCommunication(Name, StartDate, OwnerId);
        ...
        public Guid CreatePersonDocumentFormat(Guid OwnerId, string Name, DateTime StartDate, int DocumentFormatTypeId = 2)
        {
            if (!dbHelper.personDocumentFormat.GetByName(Name).Any())
                return dbHelper.personDocumentFormat.CreatePersonDocumentFormat(OwnerId, Name, StartDate, DocumentFormatTypeId);
        ...
        public Guid CreateImmigrationStatus(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.immigrationStatus.GetByName(Name).Any())
                return dbHelper.immigrationStatus.CreateImmigrationStatus(Name, StartDate, OwnerId);
        ...
        public Guid CreateReligion(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.religion.GetByName(Name).Any())
                return dbHelper.religion.CreateReligion(Name, StartDate, OwnerId);
        ...
        public Guid CreateCountry(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.country.GetByName(Name).Any())
                return dbHelper.country.CreateCountry(Name, StartDate, OwnerId);
        ...
        public Guid CreateLeavingCareEligibility(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.leavingCareEligibility.GetByName(Name).Any())
                return dbHelper.leavingCareEligibility.CreateLeavingCareEligibility(Name, StartDate, OwnerId);
        ...
        public Guid CreateUPNUnknownReason(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.upnUnknownReason.GetByName(Name).Any())
                return dbHelper.upnUnknownReason.CreateUPNUnknownReason(Name, StartDate, OwnerId);
        ...
        public Guid CreateSexualOrientation(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.sexualOrientation.GetByName(Name).Any())
                return dbHelper.sexualOrientation.CreateSexualOrientation(Name, StartDate, OwnerId);
        ...
        public Guid CreateSocialWorkerChangeReason(string Name, DateTime StartDate, Guid OwnerId)
        {
            if (!dbHelper.socialWorkerChangeReason.GetByName(Name).Any())
                return dbHelper.socialWorkerChangeReason.CreateSocialWorkerChangeReason(Name, StartDate, OwnerId);
        ...
        {
            if (!dbHelper.cPPersonAbsenceReason.GetByName(name).Any())
                return dbHelper.cPPersonAbsenceReason.CreateCPPersonAbsenceReason(name, ownerid, owningbusinessunitid, code, startdate);
        ...
        public Guid CreateCarePlanReviewOutcome(Guid CarePlanReviewOutcomeId, string Name, DateTime StartDate, DateTime? EndDate, Guid OwnerId)
        {
            if (!dbHelper.carePlanReviewOutcome.GetByName(Name).Any())
                return dbHelper.carePlanReviewOutcome.CreateCarePlanReviewOutcome(CarePlanReviewOutcomeId, Name, StartDate, EndDate, OwnerId);
        ...
        {
            if (!dbHelper.carePlanReviewOutcome.GetByName(Name).Any())
                return dbHelper.carePlanReviewOutcome.CreateCarePlanReviewOutcome(Name, StartDate, EndDate, OwnerId);
        ...
        public Guid CreateInpatientConsultantEpisodeEndReason(Guid ownerid, string Name, DateTime startdate, int code, bool inactive = false, bool DefaultForConsultantChange = false, bool DefaultForDischarge = false)
        {
            if (!dbHelper.inpatientConsultantEpisodeEndReason.GetByName(Name).Any())
                return dbHelper.inpatientConsultantEpisodeEndReason.CreateInpatientConsultantEpisodeEndReason(ownerid, Name, startdate, code, inactive, DefaultForConsultantChange, DefaultForDischarge);
        ...
        public Guid CreateRTTTreatmentFunctionCode(Guid OwnerId, string Name, DateTime StartDate)
        {
            if (!dbHelper.rttTreatmentFunctionCode.GetByName(Name).Any())
                return dbHelper.rttTreatmentFunctionCode.CreateRTTTreatmentFunctionCode(OwnerId, Name, StartDate);
        ...
        public Guid CreateCareProviderStaffRoleTypeGroup(Guid OwnerId, string Name, string Code, string GovCode, DateTime StartDate, string Description, bool Inactive = false)
        {
            if (!dbHelper.careProviderStaffRoleTypeGroup.GetByName(Name).Any())
                return dbHelper.careProviderStaffRoleTypeGroup.CreateCareProviderStaffRoleTypeGroup(OwnerId, Name, Code, GovCode, StartDate, Description, Inactive);
        ...
        public Guid CreateCPBankHolidayChargingCalendar(Guid OwnerId, string Name, string Code)
        {
            if (!dbHelper.cpBankHolidayChargingCalendar.GetByName(Name).Any())
                return dbHelper.cpBankHolidayChargingCalendar.CreateCPBankHolidayChargingCalendar(OwnerId, Name, Code);
        ...
        public Guid CreateBankHoliday(string Name, DateTime HolidayDate, string Description)
        {
            if (!dbHelper.bankHoliday.GetByHolidayDate(HolidayDate).Any())
                return dbHelper.bankHoliday.CreateBankHoliday(Name, HolidayDate, Description);
        ...
        public Guid CreateStaffReviewSetup(string Name, Guid StaffReviewFormId, DateTime ValidFrom, string Description, bool AllBusinessUnits, bool AllRoles, bool UpdateStaffReviewItem, Guid BookingTypeId)
        {
            if (!dbHelper.staffReviewSetup.GetByName(Name).Any())
                return dbHelper.staffReviewSetup.CreateStaffReviewSetup(Name, StaffReviewFormId, ValidFrom, Description, AllBusinessUnits, AllRoles, UpdateStaffReviewItem, BookingTypeId);
        ...
        {
            if (!dbHelper.systemUserEmploymentContract.GetContractsForUser(SystemUserId, CareProviderStaffRoleTypeId, EmploymentContractTypeId).Any())
                return dbHelper.systemUserEmploymentContract.CreateSystemUserEmploymentContract(SystemUserId, StartDate, CareProviderStaffRoleTypeId, OwnerId, EmploymentContractTypeId, ContractHoursPerWeek);

            return dbHelper.systemUserEmploymentContract.GetContractsForUser(SystemUserId, CareProviderStaffRoleTypeId, EmploymentContractTypeId)[0];
        }
        ...
        public Guid CreateSystemUserEmploymentContract(Guid SystemUserId, DateTime? StartDate, Guid CareProviderStaffRoleTypeId, Guid OwnerId, Guid EmploymentContractTypeId, DateTime? endDate, string description = "Test")
        {
            if (!dbHelper.systemUserEmploymentContract.GetBySystemUserId(SystemUserId).Any())
                return dbHelper.systemUserEmploymentContract.CreateSystemUserEmploymentContract(SystemUserId, StartDate, CareProviderStaffRoleTypeId, OwnerId, EmploymentContractTypeId, endDate, description);
        ...
        public Guid CreatePronouns(Guid ownerId, Guid businessUnitId, string name, int? code, DateTime startDate, bool inactive = false, bool validforexport = true)
        {
            if (!dbHelper.pronouns.GetByName(name).Any())
                return dbHelper.pronouns.CreatePronouns(ownerId, businessUnitId, name, code, startDate, inactive, validforexport);
        ...
        public Guid CreateTrainingRequirementSetup(string RecordName, Guid TrainingItemId, DateTime ValidFromDate, DateTime? ValidToDate, bool AllRoles, int StatusId, List<Guid> CareProviderStaffRoleTypeIds)
        {
            if (!dbHelper.trainingRequirementSetup.GetByName(RecordName).Any())
                return dbHelper.trainingRequirementSetup.CreateTrainingRequirementSetup(RecordName, TrainingItemId, ValidFromDate, ValidToDate, AllRoles, StatusId, CareProviderStaffRoleTypeIds);
        ...
        public Guid CreateRequirementLastChasedOutcome(string Name, Guid ownerid, DateTime StartDate, DateTime? EndDate = null)
        {
            if (!dbHelper.requirementLastChasedOutcome.GetRequirementLastChasedOutcomeByName(Name).Any())
                return dbHelper.requirementLastChasedOutcome.CreateRequirementLastChasedOutcome(Name, ownerid, StartDate, EndDate);
        ...
        public Guid CreateDemographicsTitle(string Name, DateTime startdate, Guid ownerid)
        {
            if (!dbHelper.demographicsTitle.GetByName(Name).Any())
                return dbHelper.demographicsTitle.CreateDemographicsTitle(Name, startdate, ownerid);
        ...
        public Guid CreateSystemUserAliasType(Guid ownerId, Guid OwningBusinessUnitId, string Name, DateTime startdate)
        {
            if (!dbHelper.systemUserAliasType.GetSystemUserAliasesTypeByName(Name).Any())
                return dbHelper.systemUserAliasType.CreateSystemUserAliasType(ownerId, OwningBusinessUnitId, Name, startdate);
        ...
        public Guid CreateNumericQuestion(string Question, string SubHeading)
        {
            if (!dbHelper.questionCatalogue.GetByQuestionName(Question).Any())
                return dbHelper.questionCatalogue.CreateNumericQuestion(Question, SubHeading);
        ...
        {
            if (!dbHelper.cpSuspendDebtorInvoicesReason.GetByName(Name).Any())
                return dbHelper.cpSuspendDebtorInvoicesReason.CreateCPSuspendDebtorInvoicesReason(ownerid, Name, startdate, code);
            return dbHelper.cpSuspendDebtorInvoicesReason.GetByName(Name)[0];
        }

        public Guid CreateCareProviderFinanceCode(Guid ownerid, Guid financecodelocationid, string description, string code)
        {
            if (!dbHelper.careProviderFinanceCode.GetByDescriptionAndCode(description, code).Any())
                return dbHelper.careProviderFinanceCode.CreateCareProviderFinanceCode(ownerid, financecodelocationid, description, code);
            return dbHelper.careProviderFinanceCode.GetByDescriptionAndCode(description, code)[0];
        }
        ...
        {
            if (!dbHelper.careProviderExtractType.GetByName(Name).Any())
                return dbHelper.careProviderExtractType.CreateCareProviderExtractType(OwnerId, Name, Code, GovCode, StartDate, EndDate, Inactive, ValidForExport);
            return dbHelper.careProviderExtractType.GetByName(Name)[0];
        }
        ...
        public Guid CreateTimebandSet(string Name, Guid ownerid)
        {
            if (!dbHelper.cpTimebandSet.GetByName(Name).Any())
                return dbHelper.cpTimebandSet.CreateCPTimebandSet(Name, ownerid);
            return dbHelper.cpTimebandSet.GetByName(Name)[0];
        }

        public Guid CreateTimeband(Guid ownerid, Guid cptimebandsetid, int startdayid, TimeSpan starttime, int enddayid, TimeSpan endtime)
        {
            if (!dbHelper.cpTimeband.GetByTimebandSetid(cptimebandsetid).Any())
                return dbHelper.cpTimeband.CreateCPTimeband(ownerid, cptimebandsetid, startdayid, starttime, enddayid, endtime);
            return dbHelper.cpTimeband.GetByTimebandSetid(cptimebandsetid)[0];
        }

        public Guid CreateCareProviderBandRateType(Guid ownerid, string Name, string code, DateTime startdate)
        {
            if (!dbHelper.careProviderBandRateType.GetByName(Name).Any())
                return dbHelper.careProviderBandRateType.CreateCareProviderBandRateType(ownerid, Name, code, startdate);
            return dbHelper.careProviderBandRateType.GetByName(Name)[0];
        }
        ...
        {
            if (!dbHelper.careProviderBandRateSchedule.GetByBandRateTypeIdAndName(careproviderbandratetypeid, Name).Any())
                return dbHelper.careProviderBandRateSchedule.CreateCareProviderBandRateSchedule(ownerid, Name, careproviderbandratetypeid, timefrom, timeto);
            return dbHelper.careProviderBandRateSchedule.GetByBandRateTypeIdAndName(careproviderbandratetypeid, Name)[0];
        }
        ...
        {
            if (!dbHelper.cpBankHolidayDate.GetByCPBankHolidayChargingCalendarId(CPBankHolidayChargingCalendarId, BankHolidayId).Any())
                return dbHelper.cpBankHolidayDate.CreateCPBankHolidayDate(OwnerId, CPBankHolidayChargingCalendarId, Name, BankHolidayId, CareProviderBankHolidayTypeId);
            return dbHelper.cpBankHolidayDate.GetByCPBankHolidayChargingCalendarId(CPBankHolidayChargingCalendarId, BankHolidayId)[0];
        }

        //method to create CP Bands
        ...
        {
            if (!dbHelper.cPBands.GetByName(Name).Any())
                return dbHelper.cPBands.CreateCPBand(Name, startdate, OwnerId, Code, GovCode, inactive, validforexport);
            return dbHelper.cPBands.GetByName(Name)[0];
        }
        ...
        {
            if (!dbHelper.careProviderVATCodeSetup.GetByVATCodeAndVATReference(vatcodeid, vatreference).Any())
                return dbHelper.careProviderVATCodeSetup.CreateCareProviderVATCodeSetup(ownerid, vatcodeid, vatpercentage, vatreference, startdate, enddate);
            return dbHelper.careProviderVATCodeSetup.GetByVATCodeAndVATReference(vatcodeid, vatreference)[0];
        }

        public Guid CreateCareProviderFinanceInvoicePaymentReportType(Guid OwnerId, string Name, DateTime StartDate, int? Code, string GovCode, DateTime? EndDate = null, bool Inactive = false, bool ValidForExport = false)
        {
            if (!dbHelper.careProviderFinanceInvoicePaymentReportType.GetByName(Name).Any())
                return dbHelper.careProviderFinanceInvoicePaymentReportType.CreateCareProviderFinanceInvoicePaymentReportType(OwnerId, Name, StartDate, Code, GovCode, EndDate, Inactive, ValidForExport);
            return dbHelper.careProviderFinanceInvoicePaymentReportType.GetByName(Name)[0];
        }
